...
from basicsr.arch.DIEM_arch import get_intensity
...
class BTLoss(nn.Module):
    ...
    def __init__(self, lambda_param=1e-3):
        ...
        # self.device1 = device1.to(self.device)
...
#         MADE = NTIRE_score()
#         out1, out2 = MADE(pred, target)
#         return self.loss_weight * self.criterion(out1, out2)
...
class DRLoss(nn.Module):
    ...
    def forward(self, pred, target):
        intensity_SR = get_intensity(pred)
        intensity_blur_SR = intensity_SR[0]
        intensity_noise_SR = intensity_SR[1]

        loss_blur = self.criterion(intensity_blur_SR, torch.zeros_like(intensity_blur_SR))
        loss_noise = self.criterion(intensity_noise_SR, torch.zeros_like(intensity_noise_SR))

        loss = loss_blur + loss_noise
