...
class CoordinateAttention(nn.Module):
    def __init__ (self, channel, reduction=16):
        ...
        self.relu = nn.ReLU()
        self.ln = LayerNorm2d(channel//reduction)
        self.conv_h = nn.Conv2d(in_channels=channel//reduction, out_channels=channel, kernel_size=1, stride=1, padding=0)
        self.conv_w = nn.Conv2d(in_channels=channel/reduction, out_channels=channel, kernel_size=1, stride=1, padding=0)
...
def window_partition(x, window_size):
    ...
    """
    b, h, w, c = x.shape
    # print('lllpmlpm', x.shape, window_size)
...
class CA_layer(nn.Module):
    ...
    def forward(self, x , p):
        att = self.conv_du(p[:, :, None, None])
        #### !!!!! ####
        att = torch.cat([att, att], dim=0)
        return x * att
